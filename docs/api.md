...
| 기능 | 필드 | 규칙 |
|---|---|---|
| 회원가입 | email | 필수, 공백 불가, 이메일 형식 |
| 회원가입 | password | 필수, 공백 불가, 8~72자 |
| 회원가입 | nickname | 필수, 공백 불가, 최대 100자 |
| 회원가입 | name | 필수, 공백 불가, 최대 100자 |
| 회원가입 | phone | 선택, 최대 30자 |
| 로그인 | email | 필수, 공백 불가, 이메일 형식 |
| 로그인 | password | 필수, 공백 불가 |
| 토큰 재발급 | refreshToken | 필수, 공백 불가 |
| 내 정보 수정 | nickname | 선택, 공백이면 불가, 최대 100자 |
| 내 정보 수정 | name | 선택, 공백이면 불가, 최대 100자 |
| 내 정보 수정 | phone | 선택, 최대 30자 |
| 내 정보 수정 | profileImageUrl | 선택, URL 형식 권장 |
| 상품 등록 | categoryId | 필수, 존재하는 카테고리 ID |
| 상품 등록 | tradeType | 필수, `SALE` 또는 `AUCTION` |
| 상품 등록 | title | 필수, 공백 불가, 최대 200자 |
| 상품 등록 | description | 필수, 공백 불가 |
| 상품 등록 | initialPrice | 0 이상 |
| 상품 등록 | conditionType | 필수, `NEW` 또는 `USED` |
| 상품 등록 | closeDate | `AUCTION`일 때 필수, 현재 시각 이후 |
| 상품 임시저장 | 전체 필드 | 선택, 저장 시 `isDraft=true` |
| 상품 검색 | keyword | 선택, 공백이면 전체 목록과 동일하게 처리 |
| 판매 상태 변경 | tradeStatus | 필수, `ON_SALE`, `RESERVED`, `SOLD_OUT` |
| 입찰하기 | bidPrice | 필수, 현재 입찰가보다 큰 0 이상의 정수 |
| 상품 문의 작성 | title | 필수, 공백 불가 |
| 상품 문의 작성 | contents | 필수, 공백 불가 |
| 상품 문의 수정 | contents | 필수, 공백 불가 |
| 상품 문의 답변 등록·수정 | title | 필수, 공백 불가 |
| 상품 문의 답변 등록·수정 | contents | 필수, 공백 불가 |
| 채팅 메시지 전송 | content | 필수, 공백 불가 |
| 리뷰 작성 | rating | 필수, 1~5 |
| 리뷰 작성 | content | 선택 |
| 리뷰 수정 | rating | 선택, 전달 시 1~5 |
| 리뷰 수정 | content | 선택 |
